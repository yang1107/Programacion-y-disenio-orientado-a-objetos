...
require_relative 'treasure_kind'
  ...
  badConsequence = BadConsequence.newLevelSpecificTreasures('Te atrapan para llevarte de fiesta y te dejan caer en mitad del vuelo. Descarta 1 mano visible y 1 mano oculta',0,tvp,top)
  ...
  monsters.each do |monstruos|
    if (monstruos.nivelSuperiorA(10))
      puts monstruos
    end
  end
  ...
  monsters.each do |monstruos|
    if(monstruos.MalRolloPerdidaNiveles())
      puts monstruos
    end
  end
  ...
  monsters.each do |monstruos|
    if(monstruos.BuenRolloGananciaNivelesSupA(1))
      puts monstruos
    end
  end
  ...
  monsters.each do |monstruos|
    if(monstruos.PerdidaTesoroVisibleOculto)
      puts monstruos
    end
  end
...
include Other, CombatResult
ResultTransformer.transform(CombatResult::WIN)

#EXAMEN
puts "primero #{ResultTransformer.transform(WIN)}"
puts "segundo #{ResultTransformer.transform(LOSE)}"
puts "tercero #{ResultTransformer.transform(WINGAME)}"
#FIN EXAMEN
